...
function LoggerThermo!(sys::System,comm::MPI.Comm,root::Int64)
    if sys.first_step[1] || sys.current_step[1]%sys.thermofreq==0.0
        sys.thermo_info[sys.thermologger.index[1]][1]=sys.current_step[1]
        sys.thermo_info[sys.thermologger.index[1]][2]=sys.current_temp[1]
        index=3
        if sys.thermologger.Momentum
            sys.thermo_info[sys.thermologger.index[1]][index]=sys.current_momentum[1]
            index+=1
        end
